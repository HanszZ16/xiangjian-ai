...
export function Cast() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // 命盘只走内存，不落任何存储；刷新即散
      nav('/reading', { state: { moduleId: mod.id, chart, question: values.question ?? '' } })
    } catch (e) {
      setBusy(false)
      ...
      <div className="w-full max-w-[26rem]">
        <InkReveal>
          <div className="text-center mb-12">
            <h1 className="glyph text-[28px] tracking-[0.3em] indent-[0.3em] text-[var(--fg)]">
              {mod.name}
            </h1>
            <p className="mt-3 text-[13px] text-[var(--fg-faint)]">{mod.tagline}</p>
          </div>
        </InkReveal>
        ...
        <InkReveal delay={0.8}>
          <div className="mt-12">
            ...
            <Seal onClick={() => void submit()} disabled={busy} className="w-full">
              {busy ? '排 盘 中' : '起 盘'}
            </Seal>
